...
from src.live_outlookmail_client import SetupError
from src.live_outlookmail_fetch_cli import DEFAULT_CREDENTIALS_DIR, DEFAULT_STORAGE_DIR, main


class FakeLiveOutlookMailClient:
...
class LiveOutlookMailFetchCliTests(unittest.TestCase):
    def test_live_fetch_script_runs_from_repo_root_without_pythonpath(self) -> None:
        repo_root = Path(__file__).resolve().parent.parent
        result = subprocess.run(
            [sys.executable, "scripts/live_outlookmail_fetch.py", "--help"],
            cwd=repo_root,
            capture_output=True,
            text=True,
            check=False,
        ...
        self.assertIn("Fetch live Outlook.com IMAP messages into the review queue", result.stdout)
    ...
    def test_main_defaults_to_repo_data_directories(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exit_code = main(
                ...
                stdout=stdout,
                cwd=repo_root,
                outlookmail_client_factory=lambda account_id, discovered_credentials_dir, imap_config_path, password_override: FakeLiveOutlookMailClient(
                    []
                ),
            )
    ...
    def test_main_persists_review_ready_items_for_live_outlook_messages(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exit_code = main(
                ...
                stdout=stdout,
                outlookmail_client_factory=lambda account_id, credentials_dir, imap_config_path, password_override: FakeLiveOutlookMailClient(
                    [message]
                ),
            )
    ...
    def test_main_shows_friendly_error_when_imap_config_missing(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            stdout = io.StringIO()
            stderr = io.StringIO()
            ...
            exit_code = main(
                ...
                stdout=stdout,
                stderr=stderr,
                outlookmail_client_factory=lambda account_id, credentials_dir, imap_config_path, password_override: self._raise_setup_error(),
            )

            self.assertEqual(exit_code, 2)
            self.assertIn("No Outlook IMAP config found", stderr.getvalue())

    def _raise_setup_error(self):
        raise SetupError("No Outlook IMAP config found.")
