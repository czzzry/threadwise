...
The current implementation passes 852 repository tests with 16 expected skips, all extension JavaScript behavior suites, public-data hygiene, and real browser click-throughs of the shared Proton coverage, sync, review, correction, and minimized states. The installed companion is healthy, its configured initial and teaching models are ready, Proton Mail Bridge is available, and the scheduled Proton entry point loads the same private model configuration as the companion.
